...
// FIX: Made module and action optional to support nested menu items that don't have direct permission checks.
interface NavItem {
  name: string;
  path: string;
  icon: React.ReactNode;
  subItems?: NavItem[];
  module?: 'navigation'; // To link with permissions config
  action?: string;
}
...
const permissionMap: Record<string, { module: 'navigation', action: NavigationAction }> = {
    ...
    '/operations/onhand': { module: 'navigation', action: 'viewOnhand' },
    ...
    const renderNavItems = (items: typeof navItems) => {
        ...
        return accessibleItems.map(item => {
            ...
            if (item.subItems) {
                ...
                // Don't render the parent menu if there are no accessible sub-items
    ...
    return (
        <aside className={`bg-[#0D172A] text-white transition-all duration-300 ease-in-out ${isOpen ? 'w-64' : 'w-0'} h-full flex-col flex-shrink-0 flex`}>
            ...
            <nav className="flex-1 px-4 py-4 space-y-2 overflow-y-auto custom-scrollbar">
                {isLoading ? (
                    <div className="p-4 text-center text-gray-400">Loading permissions...</div>
                ) : (
