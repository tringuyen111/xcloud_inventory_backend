...
// --- Types ---
type GoodsIssueViewData = {
  id: number;
  code: string;
  status: Database['public']['Enums']['doc_status_enum'];
  gi_type: Database['public']['Enums']['gi_type_enum'];
...
const GI_TYPES: Database['public']['Enums']['gi_type_enum'][] = ['SALE', 'RETURN', 'ADJUSTMENT', 'TRANSFER'];
...
const typeTag = (type: Database['public']['Enums']['gi_type_enum']) => {
    const colorMap: Record<Database['public']['Enums']['gi_type_enum'], string> = {
        SALE: 'blue',
        RETURN: 'volcano',
        ADJUSTMENT: 'magenta',
        TRANSFER: 'geekblue',
...
const GIListPage: React.FC = () => {
    ...
    const [goodsIssues, setGoodsIssues] = useState<GoodsIssueViewData[]>([]);
    const [loading, setLoading] = useState(false);
    const [totalCount, setTotalCount] = useState(0);
    const [pagination, setPagination] = useState({ current: 1, pageSize: 10 });
    const [sorter, setSorter] = useState<SorterResult<GoodsIssueViewData> | null>(null);
    ...
    const [partners, setPartners] = useState<FilterData[]>([]);

    const defaultColumns = ['code', 'status', 'gi_type', 'issue_mode', 'warehouse_name', 'partner_name', 'expected_date', 'created_at', 'actions'];
    const [visibleColumns, setVisibleColumns] = useState<string[]>(defaultColumns);

    // Fetch data for filters
    useEffect(() => {
        const fetchFilterData = async () => {
            const [whData, partnerData] = await Promise.all([
            ...
            if (partnerData.error) notification.error({ message: 'Lỗi tải danh sách đối tác', description: partnerData.error.message });
            else setPartners(partnerData.data);
        };
        fetchFilterData();
    }, [notification]);

    const fetchGoodsIssues = useCallback(async () => {
        setLoading(true);
        try {
            ...
            const params = {
                ...
                p_partner_id: advancedFilters.partner_id,
                p_gi_type: advancedFilters.gi_type,
            ...
            const { data, error } = await supabase.rpc('get_goods_issues_list', params);

            if (error) throw error;
            
            const result = data as { data: GoodsIssueViewData[], total_count: number };
            setGoodsIssues(result.data || []);
            setTotalCount(result.total_count || 0);

        } catch (error: any) {
            notification.error({
                message: 'Lỗi tải danh sách phiếu xuất',
    ...
    const handleTableChange = (
        ...
        filters: any, 
        newSorter: SorterResult<GoodsIssueViewData> | SorterResult<GoodsIssueViewData>[]
    ) => {
    ...
    const allColumns = useMemo(() => [
        { 
            ...
            key: 'code',
            sorter: true,
            render: (text: string, record: GoodsIssueViewData) => <Link to={`/operations/gi/${record.id}`}>{text}</Link>
        },
        { title: 'Trạng thái', dataIndex: 'status', key: 'status', sorter: true, render: statusTag },
        { title: 'Loại Phiếu xuất', dataIndex: 'gi_type', key: 'gi_type', sorter: true, render: typeTag },
        { title: 'Chế độ xuất', dataIndex: 'issue_mode', key: 'issue_mode', sorter: true, render: (text: string) => <Tag>{text}</Tag> },
        { title: 'Kho', dataIndex: 'warehouse_name', key: 'warehouse_name', sorter: true },
        { title: 'Đối tác', dataIndex: 'partner_name', key: 'partner_name', sorter: true, render: (text: string) => text || 'N/A' },
        ...
        { title: 'Ngày xuất', dataIndex: 'issued_date', key: 'issued_date', sorter: true, render: (text: string) => text ? dayjs(text).format('DD/MM/YYYY') : '-' },
        ...
        { title: 'Người cập nhật', dataIndex: 'updated_by_name', key: 'updated_by_name' },
        {
            ...
            width: 100,
            render: (_: any, record: GoodsIssueViewData) => (
                <Space size="small">
                    <Tooltip title="Xem chi tiết">
                        <button className="table-action-button" onClick={() => navigate(`/operations/gi/${record.id}`)}>
                            <EyeOutlined />
                        </button>
                    </Tooltip>
                    {record.status === 'DRAFT' && (
                        <Can module="operations" action="execute">
                            <Tooltip title="Chỉnh sửa">
                                <button className="table-action-button" onClick={() => navigate(`/operations/gi/${record.id}/edit`)}>
                                    <EditOutlined />
                                </button>
                            </Tooltip>
                        </Can>
            ...
            <Form form={form} layout="vertical" onFinish={handleFilterFinish}>
                <Form.Item name="warehouse_id" label="Kho">
                 ...
                 <Form.Item name="partner_id" label="Đối tác">
                    <Select
                        showSearch placeholder="Chọn đối tác" optionFilterProp="children"
                        filterOption={(input, option) => (option?.label ?? '').toLowerCase().includes(input.toLowerCase())}
                        options={partners.map(p => ({ value: p.id, label: p.name }))} allowClear
                    />
                </Form.Item>
                ...
                <Form.Item name="gi_type" label="Loại phiếu xuất">
                    <Select placeholder="Chọn loại phiếu xuất" mode="multiple" allowClear>
                        {GI_TYPES.map(type => <Select.Option key={type} value={type}>{type}</Select.Option>)}
                    </Select>
                </Form.Item>
        ...
        <Card>
            ...
            <div className="flex justify-between items-center mb-4">
                <Space>
                    ...
                    <Input
                        placeholder="Tìm kiếm theo Mã phiếu, Kho, Đối tác..."
