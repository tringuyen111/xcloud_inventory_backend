...
const LocationsListPage: React.FC = () => {
    ...
    const getColumns = () => {
        const actionMenu = (record: Location) => (
            <Menu>
                <Menu.Item key="1" icon={<EyeOutlined />} onClick={() => navigate(`/master-data/locations/${record.id}`)}>View</Menu.Item>
                <Menu.Item key="2" icon={<EditOutlined />} onClick={() => navigate(`/master-data/locations/${record.id}`)}>Edit</Menu.Item>
                <Menu.Divider />
    ...
    return (
        <Card>
            ...
            <Table
                ...
                onRow={(record) => ({ onDoubleClick: () => navigate(`/master-data/locations/${record.id}`)})}
            />
